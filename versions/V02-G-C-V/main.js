...
function addObject(x=Math.floor(Math.random() * (window.innerWidth - 200))+100, y=Math.floor(Math.random() * (window.innerHeight - 200))+100, v=[2 - Math.floor(Math.random() * 4), 2 - Math.floor(Math.random() * 4)]) {
    var obj = new RigidBody(world, 'circle', x, y, Math.floor(Math.random() * 20)+5, v, Math.floor(Math.random() * 1000));
}
