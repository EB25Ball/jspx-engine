...
function physics(render) {
    ...
    var initialstate = RigidBody.allInstances;
    // gravity
    ...
    //move
    var moving = [];
    var initialstate2 = RigidBody.allInstances;
    RigidBody.allInstances.forEach(obj => {
        RigidBody.allInstances.forEach( col => {
            ...
            if (hypot < 0) {hypot *= -1;console.log('oops');}
            if (hypot <= obj.rad + col.rad) {
                if (col != obj) {
                    ...
                    if (obj.pos[0] > window.innerWidth/2 && obj.pos[1] > window.innerHeight/2) {
                        //bottom right
                        obj.pos[0] == col.pos[0] - col.rad - obj.rad;
                        obj.pos[1] == col.pos[1] - col.rad - obj.rad;
                    } else if (obj.pos[0] > window.innerWidth/2 && obj.pos[1] <= window.innerHeight/2) {
                        //top right
                        obj.pos[0] == col.pos[0] - col.rad - obj.rad;
                        obj.pos[1] == col.pos[1] + col.rad + obj.rad;
                    } else if (obj.pos[0] <= window.innerWidth/2 && obj.pos[1] > window.innerHeight/2) {
    ...
    RigidBody.allInstances.forEach((obj, oindex) => {
        RigidBody.allInstances.forEach((col, cindex) => {
            var hypot = Math.hypot(obj.pos[0] - col.pos[0], obj.pos[1] - col.pos[1]);
            if (hypot <= obj.rad + col.rad && obj != col) {
                obj.pos = initialstate2[oindex].pos;
                col.pos = initialstate2[cindex].pos;
            }
        });
    });
    RigidBody.allInstances.forEach((obj, oindex) => {
        RigidBody.allInstances.forEach((col, cindex) => {
            var hypot = Math.hypot(obj.pos[0] - col.pos[0], obj.pos[1] - col.pos[1]);
            if (hypot <= obj.rad + col.rad && obj != col) {
                obj.pos = initialstate[oindex].pos;
                col.pos = initialstate[cindex].pos;
            }
        });
    });
...
c.addEventListener("click", function(e) { 
    ...
    mouseCoords = [cX, cY];
    addObject(mouseCoords[0], mouseCoords[1], /*[0, 0]/**/[2 - Math.floor(Math.random() * 4), 2 - Math.floor(Math.random() * 4)]/**/, (RigidBody.allInstances.length + 1) * 10);
});
